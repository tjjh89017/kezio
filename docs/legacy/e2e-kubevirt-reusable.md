...
e2e scenario. `.github/workflows/main.yaml`'s `e2e-deploy` job runs the
...
`e2e-deploy` stands up a KubeVirt + Multus cluster, ingests a real disk
...
## Composite actions used by `e2e-deploy`

A representative subset, not the full list - the register phase (PXE,
agent registration, MAC assertions) is unchanged from the retired
netboot lane; the deploy phase below it adds the image-service/
opentracker setup and the deploy/verify/assertion steps. Each action's
own `action.yml` documents its inputs.
...
| `install-kubevirtbmc` | Installs KubeVirtBMC, the Redfish-over-KubeVirt BMC this lane drives. |
| `create-provisioning-nads` | Creates the provisioning `NetworkAttachmentDefinition`s. |
| `deploy-existing-dhcp` | Deploys a dnsmasq pod standing in for a segment's existing DHCP server. |
| `deploy-bootd` | Deploys kezio-bootd on the provisioning bridge. |
| `provision-target-vm-via-bmc` | Powers the target VM on through its `VirtualMachineBMC`/Redfish endpoint. |
| `build-deploy-test-image` | Builds a real OS test image (guest-agent injection included), uploads it, and waits for the `Image` to reach `Ready`. |
| `deploy-machine` | Sets a `Machine`'s deploy intent and waits for it to reach `Provisioned`. |
| `verify-deploy` | Verifies the deployed disk actually booted: controller status, guest-agent connection, and a byte-for-byte content check. |
...
- **Long-running.** `e2e-deploy` budgets 120 minutes: cluster+addon
