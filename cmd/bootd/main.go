...
func main() {
	...
	flag.StringVar(&probeAddr, "health-probe-bind-address",
		fmt.Sprintf(":%d", bootd.DefaultHealthProbePort), "The address the probe endpoint binds to.")
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		// boot segment, not race other sites' bootd instances for a
...
//   - BOOTD_AGENT_UPSTREAM_URL: the in-cluster agent server's base
//     URL, for example
