...
// internal/ingest package doc comment).
...
// the internal/ingest package doc comment on why ingest slices
// partitions out first instead of attaching the whole disk). This needs
// no elevated privilege: partclone opens a regular file exactly like a
// block device, so no privileged access is required.
