...
// /register and a "wait" /next, recording whether /register was called
// before the first /next - the wiring Run is supposed to guarantee.
func newLifecycleTestServer(t *testing.T) (*httptest.Server, *atomic.Bool, *atomic.Bool) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case strings.HasSuffix(r.URL.Path, agentapi.RegisterPath):
...
// catch a wiring bug between them (registerWithRetry never invoked,
// cfg fields dropped on the way in); only driving Run itself can.
func TestRun_CollectRegisterPollWiredTogether(t *testing.T) {
	...
	if !registered.Load() {
		t.Fatal("Run never called /register")
	}
	if nextBeforeRegister.Load() {
		t.Fatal("Run polled /next before registering")
	}
...
// failNTimesThenRegister is a controller stub whose /register handler
// fails the first n requests with a 500, then succeeds.
...
func TestRegisterWithRetry_RetriesThenSucceeds(t *testing.T) {
	...
	start := time.Now()
	result, err := registerWithRetry(context.Background(), client, cfg, &keziov1alpha1.MachineHardwareStatus{})
...
func TestRegisterWithRetry_CtxCancelledReturnsPromptly(t *testing.T) {
	...
	start := time.Now()
	_, err := registerWithRetry(ctx, client, cfg, &keziov1alpha1.MachineHardwareStatus{})
	elapsed := time.Since(start)
