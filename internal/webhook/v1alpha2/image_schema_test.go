...
var _ = Describe("Image CRD schema", func() {
	...
	newImage := func() *keziov1alpha2.Image {
		imgCount++
	...
	It("rejects duplicate slot numbers", func() {
		img := newImage()
		img.Spec.Layout.Slots = append(img.Spec.Layout.Slots, keziov1alpha2.ImageSlot{
			Number: 1,
			Role:   keziov1alpha2.PartitionRoleData,
			FSType: "ext4",
		})
	...
	It("rejects a spec update after creation", func() {
		...
		img.Spec.OSFamily = keziov1alpha2.OSFamilyWindows
