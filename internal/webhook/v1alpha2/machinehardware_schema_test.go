...
var _ = Describe("MachineHardware CRD schema", func() {
	...
	newMachineHardware := func() *keziov1alpha2.MachineHardware {
		hardwareCount++
		return &keziov1alpha2.MachineHardware{
	...
	It("admits a MachineHardware with a full disk and nic inventory", func() {
		...
		hw.Spec.Disks = []keziov1alpha2.MachineHardwareDisk{{
		...
		hw.Spec.Nics = []keziov1alpha2.MachineHardwareNIC{{
			Name:       "eth0",
	...
	It("rejects more than 64 disks", func() {
		hw := newMachineHardware()
		for i := 0; i < 65; i++ {
			hw.Spec.Disks = append(hw.Spec.Disks, keziov1alpha2.MachineHardwareDisk{
