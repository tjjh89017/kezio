...
func SetupDeployRunWebhookWithManager(mgr ctrl.Manager) error {
	return ctrl.NewWebhookManagedBy(mgr).For(&keziov1alpha2.DeployRun{}).
...
// +kubebuilder:webhook:path=/validate-kezio-kojuro-date-v1alpha2-deployrun,mutating=false,failurePolicy=fail,sideEffects=None,groups=kezio.kojuro.date,resources=deployruns,verbs=create;update,versions=v1alpha2,name=vdeployrun-v1alpha2.kb.io,admissionReviewVersions=v1
...
func (v *DeployRunCustomValidator) ValidateCreate(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	deployrun, ok := obj.(*keziov1alpha2.DeployRun)
	if !ok {
...
func (v *DeployRunCustomValidator) ValidateUpdate(_ context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	deployrun, ok := newObj.(*keziov1alpha2.DeployRun)
	if !ok {
