...
import (
	...
	// Blank-imported so their init() registers "redfish"/"ipmi" with
	// internal/bmc's registry - the webhook's scheme check needs a real
	// driver package linked in, same as the manager binary.
...
var _ = BeforeSuite(func() {
	...
	var err error
	err = keziov1alpha3.AddToScheme(scheme.Scheme)
	...
	err = SetupPostHookWebhookWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())

	err = SetupSiteWebhookWithManager(mgr)
	Expect(err).NotTo(HaveOccurred())
	...
	// exist.
	Expect(k8sClient.Create(ctx, &keziov1alpha3.Site{
		ObjectMeta: metav1.ObjectMeta{Name: "site-a", Namespace: "default"},
	})).To(Succeed())
	Expect(k8sClient.Create(ctx, &keziov1alpha3.Subnet{
