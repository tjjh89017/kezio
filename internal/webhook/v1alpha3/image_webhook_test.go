...
var _ = Describe("Image Webhook", func() {
	var (
		obj       *keziov1alpha3.Image
		oldObj    *keziov1alpha3.Image
	...
	Context("slot sizeBytes versus referenced PartitionContent lastExtentEnd", func() {
		...
		newPartitionContent := func(lastExtentEnd int64) *keziov1alpha3.PartitionContent {
			pcCount++
		...
		newImage := func(name string, slots ...keziov1alpha3.ImageSlot) *keziov1alpha3.Image {
		...
		It("admits a slot whose sizeBytes equals the content's lastExtentEnd", func() {
			...
			img := newImage("image-slot-size-equal", keziov1alpha3.ImageSlot{
				Number:     1,
				Role:       keziov1alpha3.PartitionRoleData,
				ContentRef: &keziov1alpha3.NameRef{Name: pc.Name},
				SizeBytes:  2048,
			})
		...
		It("admits a slot whose sizeBytes exceeds the content's lastExtentEnd", func() {
			pc := newPartitionContent(2048)
			img := newImage("image-slot-size-larger", keziov1alpha3.ImageSlot{
				Number:     1,
				Role:       keziov1alpha3.PartitionRoleData,
				ContentRef: &keziov1alpha3.NameRef{Name: pc.Name},
				SizeBytes:  4096,
		...
		It("denies a slot whose sizeBytes is smaller than the content's lastExtentEnd", func() {
			...
			img := newImage("image-slot-size-smaller", keziov1alpha3.ImageSlot{
				Number:     3,
				Role:       keziov1alpha3.PartitionRoleData,
				ContentRef: &keziov1alpha3.NameRef{Name: pc.Name},
				SizeBytes:  2048,
		...
		It("admits a slot whose contentRef does not resolve, with a warning naming it", func() {
			img := newImage("image-slot-missing-referent", keziov1alpha3.ImageSlot{
				Number:     1,
				Role:       keziov1alpha3.PartitionRoleData,
				ContentRef: &keziov1alpha3.NameRef{Name: "pc-0000000000000000000000000000000000000000"},
				SizeBytes:  2048,
			})
		...
		It("admits a slot with contentRef but no declared sizeBytes, with a warning naming it", func() {
			pc := newPartitionContent(4096)
			img := newImage("image-slot-no-size", keziov1alpha3.ImageSlot{
				Number:     1,
				Role:       keziov1alpha3.PartitionRoleData,
				ContentRef: &keziov1alpha3.NameRef{Name: pc.Name},
			})
		...
		It("denies when only one of several slots violates the rule, naming the violating slot", func() {
			...
			img := newImage("image-slot-multi",
				keziov1alpha3.ImageSlot{
					Number:     1,
					Role:       keziov1alpha3.PartitionRoleESP,
					ContentRef: &keziov1alpha3.NameRef{Name: okContent.Name},
					SizeBytes:  2048,
				},
				keziov1alpha3.ImageSlot{
					Number:     2,
					Role:       keziov1alpha3.PartitionRoleData,
					ContentRef: &keziov1alpha3.NameRef{Name: badContent.Name},
					SizeBytes:  2048,
				},
			)
			_, err := validator.ValidateCreate(ctx, img)
		...
		It("denies a slot whose contentRef.namespace differs from the Image's own namespace", func() {
			pc := newPartitionContent(2048)
			img := newImage("image-slot-cross-namespace", keziov1alpha3.ImageSlot{
				Number:     1,
				Role:       keziov1alpha3.PartitionRoleData,
				ContentRef: &keziov1alpha3.NameRef{Name: pc.Name, Namespace: "other-namespace"},
				SizeBytes:  2048,
			})
			_, err := validator.ValidateCreate(ctx, img)
			Expect(err).To(HaveOccurred())
		...
		It("admits a slot whose contentRef.namespace is explicitly set to the Image's own namespace", func() {
			pc := newPartitionContent(2048)
			img := newImage("image-slot-same-namespace-explicit", keziov1alpha3.ImageSlot{
				Number:     1,
				Role:       keziov1alpha3.PartitionRoleData,
				ContentRef: &keziov1alpha3.NameRef{Name: pc.Name, Namespace: "default"},
				SizeBytes:  2048,
			})
			Expect(validator.ValidateCreate(ctx, img)).Error().NotTo(HaveOccurred())
		})
		...
		It("ignores blank and swap/uuid slots that carry no contentRef", func() {
			img := newImage("image-slot-blank-and-swap",
				...
				keziov1alpha3.ImageSlot{
					Number:    2,
					Role:      keziov1alpha3.PartitionRoleSwap,
	...
	Context("postHookRefs versus the referenced PostHook's osFamily", func() {
		newPostHook := func(name string, steps ...keziov1alpha3.PostHookStep) *keziov1alpha3.PostHook {
			ph := &keziov1alpha3.PostHook{
				ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: "default"},
				Spec:       keziov1alpha3.PostHookSpec{Steps: steps},
			}
		...
		newImageWithHooks := func(name string, refs ...keziov1alpha3.NameRef) *keziov1alpha3.Image {
			return &keziov1alpha3.Image{
				ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: "default"},
				Spec: keziov1alpha3.ImageSpec{
					Layout: keziov1alpha3.ImageDiskLayout{
		...
		It("admits an Image whose referenced PostHook is compatible", func() {
			hook := newPostHook("hook-compatible", keziov1alpha3.PostHookStep{
				OSFamily: keziov1alpha3.OSFamilyLinux,
				Script:   &keziov1alpha3.PostHookScriptSource{Script: "echo hi"},
			})
			img := newImageWithHooks("image-posthook-compatible", keziov1alpha3.NameRef{Name: hook.Name})
			Expect(validator.ValidateCreate(ctx, img)).Error().NotTo(HaveOccurred())
		})

		It("denies an Image whose referenced hook step declares a mismatched osFamily, naming the step's field path", func() {
			hook := newPostHook("hook-osfamily-mismatch", keziov1alpha3.PostHookStep{
				OSFamily: keziov1alpha3.OSFamilyWindows,
				Script:   &keziov1alpha3.PostHookScriptSource{Script: "echo hi"},
			})
			img := newImageWithHooks("image-posthook-osfamily-mismatch", keziov1alpha3.NameRef{Name: hook.Name})
			_, err := validator.ValidateCreate(ctx, img)
			Expect(err).To(HaveOccurred())
		...
		It("admits an Image whose referenced PostHook does not exist yet, with a warning naming it", func() {
			img := newImageWithHooks("image-posthook-missing", keziov1alpha3.NameRef{Name: "does-not-exist"})
			warnings, err := validator.ValidateCreate(ctx, img)
			Expect(err).NotTo(HaveOccurred())
		...
		It("denies a postHookRefs entry naming a namespace other than the Image's own", func() {
			img := newImageWithHooks("image-posthook-cross-namespace",
				keziov1alpha3.NameRef{Name: "hook-elsewhere", Namespace: "other-namespace"})
			_, err := validator.ValidateCreate(ctx, img)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("spec.postHookRefs[0]"))
			Expect(err.Error()).To(ContainSubstring("other-namespace"))
