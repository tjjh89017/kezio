...
var _ = Describe("Site Webhook", func() {
	var (
		obj       *keziov1alpha3.Site
		oldObj    *keziov1alpha3.Site
	...
	newSubnet := func(siteRefName string) *keziov1alpha3.Subnet {
		subnetSeq++
		...
		subnet := &keziov1alpha3.Subnet{
			...
			Spec: keziov1alpha3.SubnetSpec{
				SiteRef:         keziov1alpha3.NameRef{Name: siteRefName},
				CIDR:            "192.0.2.0/24",
	...
	Context("When creating or updating Site under Validating Webhook", func() {
		...
		It("denies a non-seeding Site carrying a tracker", func() {
			...
			obj.Spec.Tracker = keziov1alpha3.SiteTracker{IP: "192.0.2.9"}
		...
		It("denies a Site whose seederSubnetRef names a Subnet that does not exist", func() {
			...
			obj.Spec.SeederSubnetRef = &keziov1alpha3.NameRef{Name: "no-such-subnet"}
			obj.Spec.Tracker = keziov1alpha3.SiteTracker{IP: "192.0.2.9"}
			Expect(validator.ValidateCreate(ctx, obj)).Error().To(HaveOccurred())
		})
		...
		It("denies a Site whose seederSubnetRef names a Subnet belonging to another Site", func() {
			...
			obj.Spec.SeederSubnetRef = &keziov1alpha3.NameRef{Name: subnet.Name}
			obj.Spec.Tracker = keziov1alpha3.SiteTracker{IP: "192.0.2.9"}
		...
		It("admits a valid seeding Site with tracker.externalURL", func() {
			...
			obj.Spec.SeederSubnetRef = &keziov1alpha3.NameRef{Name: subnet.Name}
			obj.Spec.Tracker = keziov1alpha3.SiteTracker{ExternalURL: "http://tracker.example.com:6969/announce"}
			Expect(validator.ValidateCreate(ctx, obj)).Error().NotTo(HaveOccurred())
		})
		...
		It("denies a Site whose seederSubnetRef names a Subnet whose siteRef names a same-named Site in another namespace", func() {
			...
			foreignSiteName := "hq"
			foreignSite := &keziov1alpha3.Site{
			...
			subnet := &keziov1alpha3.Subnet{
				...
				Spec: keziov1alpha3.SubnetSpec{
					SiteRef:         keziov1alpha3.NameRef{Name: foreignSiteName},
					CIDR:            "192.0.2.0/24",
					BootdServerIP:   "192.0.2.2",
					BootdNetworkRef: &keziov1alpha3.NameRef{Name: "bootd-net"},
					DHCP: &keziov1alpha3.SubnetDHCP{
			...
			obj.Spec.SeederSubnetRef = &keziov1alpha3.NameRef{Name: subnet.Name, Namespace: otherNS.Name}
			obj.Spec.Tracker = keziov1alpha3.SiteTracker{IP: "192.0.2.9"}
		...
		DescribeTable("tracker.externalURL validation",
			func(externalURL string, shouldSucceed bool) {
				...
				obj.Spec.SeederSubnetRef = &keziov1alpha3.NameRef{Name: subnet.Name}
				obj.Spec.Tracker = keziov1alpha3.SiteTracker{ExternalURL: externalURL}
				_, err := validator.ValidateCreate(ctx, obj)
				if shouldSucceed {
					Expect(err).NotTo(HaveOccurred())
				} else {
