...
var _ = Describe("DeployRun CRD schema", func() {
	...
	newDeployRun := func() *keziov1alpha3.DeployRun {
		runCount++
	...
	It("rejects a DeployRun with no machineRef", func() {
		r := newDeployRun()
		r.Spec.MachineRef = keziov1alpha3.NameRef{}
	...
	It("rejects more than 32 dataImages entries", func() {
		r := newDeployRun()
		for i := 0; i < 33; i++ {
			r.Spec.DataImages = append(r.Spec.DataImages, keziov1alpha3.MachineDataImage{
				ImageRef: keziov1alpha3.NameRef{Name: fmt.Sprintf("data-image-%d", i)},
			})
		}
		Expect(k8sClient.Create(ctx, r)).To(HaveOccurred())
	})
	...
	It("rejects a spec update after creation", func() {
		...
		r.Spec.MachineRef = keziov1alpha3.NameRef{Name: "machine-b"}
	...
	It("admits a status update that leaves spec untouched", func() {
		...
		r.Status.Phase = keziov1alpha3.DeployRunPhasePartitioning
