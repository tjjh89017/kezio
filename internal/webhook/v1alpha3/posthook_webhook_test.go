...
var _ = Describe("PostHook Webhook", func() {
	var (
		obj       *keziov1alpha3.PostHook
		oldObj    *keziov1alpha3.PostHook
		validator PostHookCustomValidator
	)

	newValidPostHook := func() *keziov1alpha3.PostHook {
	...
	Context("When creating or updating PostHook under Validating Webhook", func() {
		...
		It("denies a step with no kind set on create", func() {
			obj.Spec.Steps = []keziov1alpha3.PostHookStep{{}}
		...
		It("admits a script placeholder that references a declared param", func() {
			obj.Spec.Params = []keziov1alpha3.PostHookParam{{Name: "greeting"}}
			obj.Spec.Steps = []keziov1alpha3.PostHookStep{
		...
		It("admits an allowed builtin param with a declared placeholder", func() {
			obj.Spec.Params = []keziov1alpha3.PostHookParam{{Name: "diskName"}}
			obj.Spec.Steps = []keziov1alpha3.PostHookStep{
				{Builtin: &keziov1alpha3.PostHookBuiltinStep{
					Name:   keziov1alpha3.BuiltinStepInstallRemovableFallback,
		...
		It("denies a duplicate param name", func() {
			obj.Spec.Params = []keziov1alpha3.PostHookParam{{Name: "a"}, {Name: "a"}}
			_, err := validator.ValidateCreate(ctx, obj)
			Expect(err).To(HaveOccurred())
	...
	Context("admission round-trip through the webhook server", func() {
		It("admits a valid PostHook", func() {
			created := &keziov1alpha3.PostHook{
		...
		It("denies a script step whose placeholder does not reference a declared param", func() {
			candidate := &keziov1alpha3.PostHook{
				ObjectMeta: metav1.ObjectMeta{
