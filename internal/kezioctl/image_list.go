...
func ImageList(ctx context.Context, c client.Client, opts ImageListOptions) ([]keziov1alpha1.Image, error) {
...
func WriteImageList(w io.Writer, images []keziov1alpha1.Image, allNamespaces bool) error {
	tw := tabwriter.NewWriter(w, 0, 8, 2, ' ', 0)

	if allNamespaces {
		_, _ = fmt.Fprintln(tw, "NAMESPACE\tNAME\tSTATE\tBOOTABLE\tOSFAMILY\tAGE")
	} else {
		_, _ = fmt.Fprintln(tw, "NAME\tSTATE\tBOOTABLE\tOSFAMILY\tAGE")
	}
	...
	for _, img := range images {
		...
		if state == "" {
			state = keziov1alpha1.ImageStatePending
		}
		age := duration.ShortHumanDuration(now.Sub(img.CreationTimestamp.Time))

		if allNamespaces {
			_, _ = fmt.Fprintf(tw, "%s\t%s\t%s\t%t\t%s\t%s\n",
				img.Namespace, img.Name, state, img.Spec.EffectiveBootable(), img.Spec.EffectiveOSFamily(), age)
		} else {
			_, _ = fmt.Fprintf(tw, "%s\t%s\t%t\t%s\t%s\n",
				img.Name, state, img.Spec.EffectiveBootable(), img.Spec.EffectiveOSFamily(), age)
		}
	}
