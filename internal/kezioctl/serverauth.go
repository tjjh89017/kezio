...
const (
	// ServerEnvVar names the image service base URL when --server is not
	// given.
...
// closer to the command line always wins over one set further away in the
// environment:
//
...
//
// The token authenticates against internal/imageservice's Authenticator,
// which loads its own side of the shared secret from a file-mounted
// Secret (see imageservice.LoadToken) - kezioctl never reads that Secret
// through the Kubernetes API itself, so it needs no RBAC on Secrets
// beyond what the caller already has to manage the CI/deployment
// pipeline that provisions the token out of band.
//
