...
// partclone.dd for an unrecognized file system), producing one content
// directory per partition in a scratch work directory and a compact
// per-partition summary (Result). It never talks to the Kubernetes API:
// the caller (cmd/ingest, and the Image controller that dispatches it)
// maps Result's per-partition summaries onto PartitionContent objects.
//
