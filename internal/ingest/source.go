...
//     staging volume image-service writes to) and used in place, with no
//     copy: the returned path points directly at the staged file, and
//     staged reports true so the caller knows to remove it from staging
//     once ingest has safely stored its content (see
//     CleanupStagedSource).
//
...
func ResolveSource(ctx context.Context, sourceURL string, downloader Downloader, staging StagingResolver, destPath string) (path string, staged bool, err error) {
	...
	case strings.HasPrefix(sourceURL, StagedURLScheme+"://"):
		if staging == nil {
			...
			// was configured with a staging volume. A kezio-staged://
			...
			return "", false, fmt.Errorf(
				"source %s requires a staging resolver, but none is configured: "+
					"set STAGING_ROOT on the ingest Job",
				sourceURL)
		}
		name := strings.TrimPrefix(sourceURL, StagedURLScheme+"://")
	...
	default:
		return "", false, fmt.Errorf("unsupported source URL scheme in %q: expected http://, https://, or %s://", sourceURL, StagedURLScheme)
	}
}
...
func StagedNameFromURL(sourceURL string) string {
	return strings.TrimPrefix(sourceURL, StagedURLScheme+"://")
}
