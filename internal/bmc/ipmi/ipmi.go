...
// negotiation) natively, so this driver needs no external binary. There is
// deliberately no ipmitool-backed fallback driver in kezio: this package
// is the only IPMI driver.
//
// # Command mapping
//
...
// held across calls (same as redfish), since a controller connects
// briefly and disconnects often many times an hour and bmc.BMC has no
// Close hook to release one.
//
...
//     answer the soft shutdown above (matches redfish's ForceOff).
...
//     Flags) with BootDeviceSelectorForcePXE + BIOSBootTypeEFI + persist=false,
//     so the override applies to exactly the next boot.
//
...
// connect is the bmc.Driver registered for the "ipmi" scheme. It does not
// itself talk to the BMC - any unreachable-host or bad-credentials error
// surfaces from the first method call instead of from Connect.
