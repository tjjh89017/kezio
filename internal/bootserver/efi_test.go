...
func writeEFIFixtures(t *testing.T, dir string) {
	t.Helper()
	if err := os.WriteFile(filepath.Join(dir, ShimFilename), []byte("shim-bytes"), 0o600); err != nil {
		t.Fatalf("WriteFile shim: %v", err)
	}
	if err := os.WriteFile(filepath.Join(dir, GrubFilename), []byte("grub-image-bytes"), 0o600); err != nil {
...
func TestHandleEFI_SupportsRangeRequests(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, srv.URL+"/boot/http/"+ShimFilename, nil)
	if err != nil {
