...
type Config struct {
	...
	ServerURL string
	// AgentServerURL is internal/agentserver's externally reachable base
	// URL, used as the kezio.server= cmdline value the agent reads its
	// registration endpoint from. Empty means ServerURL - only correct
	// when both servers genuinely share the same address; otherwise it
	// silently sends every agent registration to a server that never
	// mounts agentserver's routes, failing without a server-side error
	// (see renderNetBootConfig's doc comment).
