...
// client, no I/O, so the matching and distinct-disk rules are
...
// ErrNoMatch means no disk in the inventory satisfies the given hints (or
// the inventory was empty). Callers must not fall back to picking a disk
// arbitrarily.
var ErrNoMatch = errors.New("no disk matches the hints")

// ErrAmbiguous means more than one disk in the inventory satisfies the
// given hints (or the hints were absent and more than one disk is
// present). Callers must stop before any write and ask for a hint that
// narrows the match to one disk.
var ErrAmbiguous = errors.New("multiple disks match the hints")
...
// one disk resolves, zero is ErrNoMatch, more than one is ErrAmbiguous
...
func Match(disks []keziov1alpha3.MachineHardwareDisk, hints *keziov1alpha3.TargetDiskHints) (*keziov1alpha3.MachineHardwareDisk, error) {
	if !hasAnyHint(hints) {
		switch len(disks) {
		case 0:
			return nil, fmt.Errorf("%w: no disks reported", ErrNoMatch)
		case 1:
			d := disks[0]
			return &d, nil
		default:
			return nil, fmt.Errorf("%w: %d disks present; a targetDisk hint is required: %s",
				ErrAmbiguous, len(disks), describeDisks(disks))
		}
	}
	...
	switch len(matches) {
	case 0:
		return nil, fmt.Errorf("%w: no disk matches the hints", ErrNoMatch)
	case 1:
		return &matches[0], nil
	default:
		return nil, fmt.Errorf("%w: %d disks match the hints; refine them: %s",
			ErrAmbiguous, len(matches), describeDisks(matches))
	}
}
...
// to the same physical disk. Disk identity is the serial number when the
...
func CheckDistinct(selections []Selection) error {
	...
	for _, s := range selections {
		...
		if prev, ok := firstLabel[id]; ok {
			return fmt.Errorf("%w: target disk conflict: %s and %s both resolve to disk %s",
				ErrAmbiguous, prev, s.Label, describeIdentity(*s.Disk))
		}
