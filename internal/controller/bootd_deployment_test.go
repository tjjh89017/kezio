...
// testSubnetName is every testSubnet fixture's fixed name: the
// envtest specs in subnet_bootd_envtest_test.go assert against the
// deterministic Deployment name it produces (bootdDeploymentName),
// so it is a shared constant rather than a per-call parameter.
const testSubnetName = "rack-1"

func testSubnet(namespace string, mutate ...func(*keziov1alpha1.Subnet)) *keziov1alpha1.Subnet {
...
// lands in the Subnet's own namespace (SubnetSpec.BootdNetworkRef's doc
// comment - decision 13) and is named deterministically from the
// Subnet's name.
...
// controller-default BOOTD_DHCP_INTERFACE/BOOTD_TFTP_DIR
// config/bootd/deployment.yaml also pins) and that the agent/boot
// upstream URLs only appear when cfg sets them.
func TestBuildBootdDeploymentEnv(t *testing.T) {
	...
	t.Run("lease mode carries the range bounds", func(t *testing.T) {
		leaseSubnet := testSubnet("site-hq", func(s *keziov1alpha1.Subnet) {
		...
		if got, _ := envValue(env, "BOOTD_LEASE_RANGE_START"); got != "192.0.2.10" {
			t.Errorf("BOOTD_LEASE_RANGE_START = %q, want %q", got, "192.0.2.10")
		}
		if got, _ := envValue(env, "BOOTD_LEASE_RANGE_END"); got != "192.0.2.20" {
			t.Errorf("BOOTD_LEASE_RANGE_END = %q, want %q", got, "192.0.2.20")
		}
	})

	t.Run("proxy mode omits range bounds even if the Subnet somehow carries them", func(t *testing.T) {
		proxySubnet := testSubnet("site-hq", func(s *keziov1alpha1.Subnet) {
			s.Spec.DHCP = keziov1alpha1.SubnetDHCP{
				Mode:            keziov1alpha1.SubnetDHCPModeProxy,
				LeaseRangeStart: "192.0.2.10",
				LeaseRangeEnd:   "192.0.2.20",
			}
		})
...
// individual keys, so a silently omitted variable shows up as a
// missing entry.
func TestBuildBootdDeploymentEnvFullSet(t *testing.T) {
	subnet := testSubnet("site-hq", func(s *keziov1alpha1.Subnet) {
		s.Spec.DHCP = keziov1alpha1.SubnetDHCP{
			Mode:            keziov1alpha1.SubnetDHCPModeLease,
			LeaseRangeStart: "192.0.2.10",
			LeaseRangeEnd:   "192.0.2.20",
	...
	want := []corev1.EnvVar{
		...
		{Name: "BOOTD_LEASE_RANGE_START", Value: "192.0.2.10"},
		{Name: "BOOTD_LEASE_RANGE_END", Value: "192.0.2.20"},
...
func TestBuildBootdDeploymentNADAnnotation(t *testing.T) {
	subnet := testSubnet("site-hq", func(s *keziov1alpha1.Subnet) {
		s.Spec.BootdNetworkRef = keziov1alpha1.NameRef{Name: "boot-nad"}
	})
	cfg := BootdDeploymentConfig{Image: "bootd:test", BootArtifactsImage: "boot-artifacts:test"}
...
// TestBuildBootdDeploymentServiceAccount checks the default and
// override paths for the pod's ServiceAccountName.
...
// replica is requested and the Deployment's own selector matches its
// pod template's labels - the "exactly one responder per broadcast
// domain" invariant this reconciler must not silently break.
...
// NodeSelector comes straight from the Subnet's own NodeSelector - the
// only mechanism (SubnetSpec.NodeSelector's doc comment) that can land a
// bootd pod on a node actually attached to the Subnet's broadcast
// domain - and that an empty or nil selector leaves the pod
// unconstrained rather than stamping an empty map.
...
// TestBuildBootdDeploymentContainerPorts pins the bootd container's
// exact ContainerPort set (proxyDHCP, PXE, reverse proxy, TFTP), so
...
// checked against bootd.DefaultHealthProbePort rather than a literal,
// so a changed default fails here instead of `kubectl rollout status`
...
// limits and requests, since a silently dropped entry would not fail
// any other test.
...
// which TFTPServer degrades to a per-file error rather than a crash.
...
// fetch-boot-artifacts's `cp` fails EACCES without the matching
// FSGroup - a failure envtest cannot observe, since it never runs a
// real kubelet.
...
// from cfg.Image, not cfg.BootArtifactsImage: envtest never pulls or
// runs an image, so a swapped reference here would only fail once a
// real pod tried to run the wrong binary.
...
// TestBuildBootdDeploymentVolumesAndMounts pins the pod's Volumes and
// the bootd container's VolumeMounts: dropping the "run" volume stays
...
func TestBuildBootdDeploymentLabelValues(t *testing.T) {
	...
	rack2 := testSubnet("site-hq", func(s *keziov1alpha1.Subnet) { s.Name = "rack-2" })
...
// bootdContainer returns dep's "bootd" container, failing the test if
// it is not found.
