...
// MiB. This is the single source of truth for the value recorded in every
// PartitionContent's spec.pieceLength - it must stay pinned, since a piece
// length that varies between ingests of identical bytes would fork them
// into different info hashes, breaking the content-addressed dedupe
// PartitionContent exists for. It is fixed on both sides of the exchange:
// ezio's own torrent-building tooling hard-codes it, and it is the same
// default piece boundary partclone cuts on while writing torrent.info.
...
// ParseTorrentInfo parses a partclone torrent.info stream: a
// "block_size:" line, a "blocks_total:" line, then "offset:"/"length:"
// pairs (one per extent) interleaved with "sha1:" lines (one per
// completed piece, wherever the piece boundary falls — not necessarily
// one per extent pair). offset/length are collected into paired Extents
// and sha1 into an independently ordered PieceHashes list; this loses no
// information because ezio's own tooling treats the file the same way,
// as three separate ordered streams rather than one interleaved record
// (see WriteTorrentInfo).

// ParseTorrentInfoBytes parses a torrent.info file already held in
// memory, rather than a path on a mounted store.
...
func ParseTorrentInfo(r io.Reader) (*TorrentInfo, error) {
	...
	// torrent.info lines are short; the scanner's default 64KiB token
	// limit needs no raising.
	lineNo := 0
	for scanner.Scan() {
		lineNo++
		line := scanner.Text()
...
// not start with prefix. Unambiguous here because every field this parser
// reads is non-empty, so an empty remainder can never be a valid match.
...
//
// This grouped layout differs from partclone's own interleaved output
// (see ParseTorrentInfo's doc comment) but is semantically equivalent:
// every reader, including ezio's own tooling, treats offset/length and
// sha1 as independent ordered streams, so grouping changes nothing
// observed.
