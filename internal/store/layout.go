...
// torrent with "name": contentName ("content"). BEP3 multi-file semantics
// resolve every file entry as save_path/<name>/<path...> — no compliant
// client can resolve a multi-file torrent's files directly under save_path
// with no extra directory level. So for AddTorrent(save_path=dir) to find
// real bytes, the extent files must live at dir/content/<hex> (see
// ContentDataDir, NestExtentFiles).
...
// typically a content's PVC mount root but may be any save_path a client
// is pointed at (e.g. a leecher's empty target directory), which is why
// this takes a plain dir rather than an InfoHash.
