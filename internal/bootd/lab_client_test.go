...
// proxyDHCP and lease mode both hinge on what a client actually
// receives, not just on what bootd renders.
//
...
func TestDnsmasqLabClient(t *testing.T) {
	...
	// DHCPOFFER never carries the boot filename - a real PXE ROM only
	// receives it from a second exchange on port 4011 (pxe-service),
	// which this lab client does not replicate. Lease mode's DHCPOFFER
	// is a plain, non-proxy offer and does carry it directly, so
	// lease-mode scenarios set this.
