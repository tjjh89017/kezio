...
// agentRoutePrefix mirrors every agentserver route (all under "/agent/"),
...
// and forwards agentRoutePrefix to internal/agentserver and
...
// internal/bootserver.Server.Start and internal/agentserver.Server.Start.
// The listener is bound explicitly with a log line on both sides, so a
// bind failure is visible in bootd's own log rather than surfacing only
// as a client timeout minutes later.
