...
// It is not a CI test: it is the in-repo entry point a containerized
// packet lab (a netns topology with a PXE-shaped test client - see
// TestDnsmasqLabClient) uses to exercise the exact code paths production
// runs - config rendering, supervision, hostsfile rewrite + SIGHUP - with
// real packets on real sockets, holding only the capabilities the pod
// grants (uid 0 with the three-capability bounding set, per
// internal/controller's buildBootdDeployment). Skipped unless
// BOOTD_LAB=1.
//
// Environment:
//
