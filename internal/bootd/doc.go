...
// or the boot config server (internal/bootserver) come into play. It also
// holds the identity constants (identity.go) internal/controller's bootd
// Deployment builder shares with the daemon itself.
//
...
// strictly read-only server allowlisting exactly ShimFilename,
// GrubFilename, and the in-memory grub.cfg (GrubConfigPath), rejecting any
// other name including path traversal - dnsmasq's TFTP would serve
// anything under its tftp-root.
//
...
// to engage for such a client at all - see render.go.
//
// Capabilities: dnsmasq requires CAP_NET_ADMIN, CAP_NET_RAW, and
// CAP_NET_BIND_SERVICE (checked at its own startup, see
// DnsmasqCapabilities). The bootd container runs as uid 0 with exactly
// those three because Kubernetes grants added capabilities to a non-root
// container's bounding set only, which execve cannot recover - see
// caps.go and internal/controller's buildBootdDeployment.
package bootd
